...
fn serializeStruct(state: *SerializerState, value: anytype, writer: *std.Io.Writer) anyerror!void {
    ...
    inline for (fields) |field| {
        ...
        if (ftype == .array or (ftype == .pointer and ftype.pointer.size == .slice)) {
            const child_t = if (ftype == .array) @typeInfo(ftype.array.child) else @typeInfo(ftype.pointer.child);
            try state.table_comp.append(state.allocator, field.name);
            if (child_t == .@"struct" and isMapType(if (ftype == .array) ftype.array.child else ftype.pointer.child)) {
                ...
                for (@field(value, field.name)) |v| {
                    var mut_v = v;
                    while (@typeInfo(@TypeOf(@typeInfo(@TypeOf(mut_v)).pointer.child)) == .pointer) mut_v = mut_v.*;
                    _ = try writer.write("[[");
                    for (0..state.table_comp.items.len - 1) |i| {
                        try writer.print("{s}.", .{state.table_comp.items[i]});
                    }
                    try writer.print("{s}]]\n", .{field.name});
                    try serializeStruct(state, mut_v.*, writer);
                }
            } else {
            ...
            var skip = false;
            if (comptime ftype == .optional) {
                if (val == null) skip = true;
            }
            if (!skip) {
                try writer.print("{s} = ", .{field.name});
                try serializeValue(state, ftype, val, writer);
                _ = try writer.write("\n");
            }
        }
    }
    ...
    inline for (fields) |field| {
        const ftype = @typeInfo(field.type);
        if (ftype == .array or (ftype == .pointer and ftype.pointer.size == .slice)) continue;
...
fn serializeMap(state: *SerializerState, value: anytype, writer: *std.Io.Writer) anyerror!void {
    {
