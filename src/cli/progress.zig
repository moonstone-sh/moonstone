...
fn installCancelHandler(cancel: *std.atomic.Value(bool)) ?std.c.Sigaction {
    ...
    return old_int; // Return one to restore (both were the same handler)
}

fn uninstallCancelHandler(old: ?std.c.Sigaction) void {
    if (builtin.os.tag == .wasi or builtin.os.tag == .freestanding) return;
    if (old) |*o| {
        std.posix.sigaction(.INT, o, null);
        std.posix.sigaction(.TERM, o, null);
    }
    global_cancel = null;
}
...
pub const ProgressEvent = union(enum) {
    /// A top-level phase began, e.g. "Solving dependencies…"
    phase_started: []const u8,
    /// A top-level phase finished, e.g. "Resolved 5 dependencies."
    phase_done: []const u8,
    /// A single package started processing (download / materialize / hash).
    package_started: []const u8,
    /// A single package finished processing.
    package_done: []const u8,
    /// Byte-level download progress for a labelled transfer.
    bytes: struct { label: []const u8, done: u64, total: ?u64 },
    /// A non-fatal warning the UI should surface.
    warning: []const u8,
    /// The worker completed successfully.
    done,
    /// The worker failed.  `msg` is a short description (error name or
...
pub const WorkerContext = struct {
    ...
    pub fn sendBytes(self: *WorkerContext, label: []const u8, done: u64, total: ?u64) void {
...
pub const ProgressUi = struct {
    ...
    pub fn init(writer: *std.Io.Writer, is_tty: bool, io: std.Io) ProgressUi {
        return .{
            .writer = writer,
            .is_tty = is_tty,
            .io = io,
    ...
    pub fn render(self: *ProgressUi) void {
        ...
        self.writer.writeAll(w.buffer[0..w.end]) catch {};
        self.writer.flush() catch {};
    }
    ...
    pub fn renderFinal(self: *ProgressUi) void {
        if (self.is_tty) {
            self.writer.print("\x1b[2K\r", .{}) catch {};
            self.writer.flush() catch {};
        }
        for (self.warnings.items) |msg| {
            self.writer.print("⚠ {s}\n", .{msg}) catch {};
        }
        if (self.warnings.items.len > 0) self.writer.flush() catch {};
    }
...
pub fn runWithProgress(
    ...
    var ui = ProgressUi.init(stdout, is_tty, io);
    defer ui.deinit();
...
pub const ProgressBackend = union(enum) {
    ...
    pub fn bytes(self: ProgressBackend, label: []const u8, done: u64, total: ?u64) void {
        switch (self) {
            .direct => {},
            .queue => |wctx| wctx.sendBytes(label, done, total),
        }
    }
...
pub fn onResolveEventProgress(ctx: ?*anyopaque, event: @import("moonstone").resolution.options.ResolveEvent) void {
    ...
    switch (event) {
        .retry => |r| {
            var buf: [256]u8 = undefined;
            const tmp = std.fmt.bufPrint(&buf, "Retrying {s} ({d}/{d})", .{ r.url, r.attempt, r.max_retries }) catch return;
            const msg = wctx.allocator.dupe(u8, tmp) catch return;
            wctx.sendWarning(msg);
        },
        .status => |s| {
            var buf: [256]u8 = undefined;
            const tmp = std.fmt.bufPrint(&buf, "{s}: {s}", .{ s.pkg_name, s.msg }) catch return;
            const msg = wctx.allocator.dupe(u8, tmp) catch return;
            wctx.sendPhase(msg);
        },
        .download_progress => |dp| {
            wctx.sendBytes(dp.pkg_name orelse dp.url, dp.downloaded_bytes, dp.total_bytes);
        },
    }
}
