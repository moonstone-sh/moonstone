...
pub const StoreDriver = struct {
    ...
    pub fn register_artifact(self: StoreDriver, allocator: std.mem.Allocator, sm: manifest.StoreManifest, path: []const u8, manifest_path: []const u8) !void {
        ...
        for (sm.provides.bin) |b| {
            try self.exec("INSERT INTO provides_bin (artifact_hash, name, path) VALUES (?, ?, ?);", .{ sm.artifact.artifact_hash, b.name, b.path });
        }
    ...
    pub fn get_provisions(self: StoreDriver, artifact_hash: []const u8) !struct {
        ...
        var bins = std.ArrayList(ArtifactProvision).empty;
        var headers = std.ArrayList(ArtifactProvision).empty;
        var libs = std.ArrayList(ArtifactProvision).empty;
        var lua_modules = std.ArrayList(ArtifactProvision).empty;
        var lua_cmodules = std.ArrayList(ArtifactProvision).empty;
        ...
        {
            var stmt: ?*c.sqlite3_stmt = null;
            if (c.sqlite3_prepare_v2(self.db, "SELECT name, path FROM provides_bin WHERE artifact_hash = ?;", -1, &stmt, null) == c.SQLITE_OK) {
                defer _ = c.sqlite3_finalize(stmt);
                _ = c.sqlite3_bind_text(stmt, 1, artifact_hash.ptr, @intCast(artifact_hash.len), transient);
                while (c.sqlite3_step(stmt) == c.SQLITE_ROW) {
    ...
    pub fn delete_artifact(self: StoreDriver, artifact_hash: []const u8) !void {
        ...
        inline for (&.{ sql_bin, sql_headers, sql_native, sql_lua_cmodule, sql_runtime, sql_artifacts }) |sql| {
            var stmt: ?*c.sqlite3_stmt = null;
    ...
    pub fn findCandidates(self: StoreDriver, query: ArtifactQuery) ![]Candidate {
        ...
        param_idx += 1;
        if (query.resolver) |r| { _ = c.sqlite3_bind_text(stmt, param_idx, r.ptr, @intCast(r.len), transient); param_idx += 1; }
        if (query.kind) |k| { const kind_str = @tagName(k); _ = c.sqlite3_bind_text(stmt, param_idx, kind_str.ptr, @intCast(kind_str.len), transient); param_idx += 1; }
        if (query.target) |t| { _ = c.sqlite3_bind_text(stmt, param_idx, t.ptr, @intCast(t.len), transient); param_idx += 1; }
        if (query.lua_abi) |a| { _ = c.sqlite3_bind_text(stmt, param_idx, a.ptr, @intCast(a.len), transient); param_idx += 1; }
        if (query.lua_api) |a| { _ = c.sqlite3_bind_text(stmt, param_idx, a.ptr, @intCast(a.len), transient); param_idx += 1; }
        if (query.runtime) |r| { _ = c.sqlite3_bind_text(stmt, param_idx, r.ptr, @intCast(r.len), transient); param_idx += 1; }
        if (query.runtime_artifact_hash) |h| { _ = c.sqlite3_bind_text(stmt, param_idx, h.ptr, @intCast(h.len), transient); param_idx += 1; }
        if (query.native_compat_required) |n| { _ = c.sqlite3_bind_int(stmt, param_idx, if (n) 1 else 0); param_idx += 1; }
