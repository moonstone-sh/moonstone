...
| Foreign pure-Lua target-profile fixture | Artifact-backed target runtime, host-only rockspec parser, shared and platform-scoped dependencies, target-profile lock verification, and replay after purging target artifacts | P1/P2/P4 for pure-Lua source rocks; CI-gated. No foreign execution or native-build claim | `tests/e2e/resolution/foreign_target_pure_lua_rocks.sh` |
